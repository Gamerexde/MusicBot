...
public class ForceRemoveCmd extends DJCommand
{
    public ForceRemoveCmd(Bot bot)
    {
        ...
        this.help = "elimina todas las entradas de un usuario de la cola";
        this.arguments = "<user>";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyError("¡Necesitas mencionar a un usuario!");
            return;
        }
        ...
        if(found.isEmpty())
        {
            event.replyError("No se pudo encontrar al usuario!");
            return;
        }
        else if(found.size()>1)
        {
            ...
            .setText("Se encontraron varios usuarios:")
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        if (count == 0)
        {
            event.replyWarning("**"+target.getName()+"** no tiene ninguna canción en la cola!");
        }
        else
        {
            event.replySuccess("Se ha eliminado `"+count+"` entradas de **"+target.getName()+"**#"+target.getDiscriminator()+".");
        }
    }
}
